...
import themesReducer, {
  ThemeCtx,
  ThemeDispatchCtx,
} from "./stateManager/themeManager";
import {
  SideBarCtx,
  SideBarDispatchCtx,
  sideBarReducer,
} from "./stateManager/sideBarManager";
          ...
          <SideBarDispatchCtx.Provider value={dispatchSideBarState}>
            <SideBarCtx.Provider value={sideBarState}>
              <BodyMeasureCtx.Provider value={bodyMeasure}>
                <body
                  ref={bodyRef}
                  className={clsx( theme.state)}
                >
                  <TopNavBar />
                  <div className="md:flex">
                    <SideBar />
                    <div
                      className={`p-4 dark:${darkText} dark:${darkbg} bg-slate-100 py-14 min-h-screen w-full`}
                    >
                      {children}
                    </div>
                  </div>
